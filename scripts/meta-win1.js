document.addEventListener('DOMContentLoaded', async () => {
    ...
    const btnPersonalizar = document.querySelector('#card-meta-win-1 .personalize-btn');
    const modal = document.getElementById('config-meta-win-modal');
    const btnClose = document.getElementById('close-config-meta-win');
    const btnSave = document.getElementById('btn-save-meta-win-config');
    const btnReset = document.getElementById('btn-reset-meta-win-config');
    ...
    const colorIds = [
        'mw1-bg-main', 'mw1-bg-counter', 'mw1-bg-meta', 
        'mw1-color-text', 'mw1-color-num', 'mw1-color-neg', 'mw1-color-border'
    ];
    ...
    const iconContainer = document.getElementById('mw1-icon-container');
    const selectedDisplay = document.getElementById('mw1-selected-display');
    const optionsList = iconContainer.querySelector('.options-list');
    const hiddenInput = document.getElementById('mw1-icon');
    ...
    const inputs = {
        font: document.getElementById('mw1-font'),
        fontSize: document.getElementById('mw1-font-size'),
        text: document.getElementById('mw1-text'),
        // icon: se maneja con hiddenInput
        borderCheck: document.getElementById('mw1-border-check'),
    };
    ...
    if (window.electronAPI) {
        try {
            const loadedData = await window.electronAPI.getWidgetData('metaWin1');
            if (loadedData) {
    ...
    function updateModalInputs() {
        ...
        if (s.bgMeta) pickers['mw1-bg-meta'].setColor(s.bgMeta);
        
        if (s.colorText) pickers['mw1-color-text'].setColor(s.colorText);
        else pickers['mw1-color-text'].setColor('#ffffff');

        if (s.colorNum) pickers['mw1-color-num'].setColor(s.colorNum);
        if (s.colorNeg) pickers['mw1-color-neg'].setColor(s.colorNeg);
        if (s.colorBorder) pickers['mw1-color-border'].setColor(s.colorBorder);
    ...
    async function saveData() {
        ...
        currentData.styles = {
            ...
            bgMain: getCol('mw1-bg-main'),
            bgCounter: getCol('mw1-bg-counter'),
            bgMeta: getCol('mw1-bg-meta'),
            colorText: getCol('mw1-color-text'),
            colorNum: getCol('mw1-color-num'),
            colorNeg: getCol('mw1-color-neg'),
            colorBorder: getCol('mw1-color-border')
        };

        if (window.electronAPI) {
            await window.electronAPI.updateWidget('metaWin1', currentData);
        }
    }
    ...
    if (btnReset) {
        btnReset.addEventListener('click', () => {
            ...
            // Resetear Pickers
            pickers['mw1-bg-main'].setColor('#000000');
            pickers['mw1-bg-counter'].setColor('#1d1d1d');
            pickers['mw1-bg-meta'].setColor('#1d1d1d');
            pickers['mw1-color-text'].setColor('#ffffff');
            pickers['mw1-color-num'].setColor('#ffffff');
            pickers['mw1-color-neg'].setColor('#ff0000');
            pickers['mw1-color-border'].setColor('#000000');
